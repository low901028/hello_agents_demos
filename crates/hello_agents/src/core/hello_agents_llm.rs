...
use super::llm_adapter::{create_adapter, LLMAdapter};
use super::llm_response::{LlmResponse, LlmToolResponse, StreamStats};
...
pub struct HelloAgentsLLM {
    ...
    pub timeout: usize,
    pub adapter: Arc<Box<dyn LLMAdapter>>,
    pub last_call_stats: Arc<StdMutex<Option<StreamStats>>>,
...
impl HelloAgentsLLM {
    pub fn new(
        ...
        let model = model
            ...
            .ok_or_else(|| {
                HelloAgentException::config("必须提供模型名称 (model 或 LLM_MODEL_ID)")
            })?;
        let api_key = api_key
            .map(|s| s.to_string())
            .or_else(|| env::var("LLM_API_KEY").ok())
            .ok_or_else(|| {
                HelloAgentException::config("必须提供API密钥 (api_key 或 LLM_API_KEY)")
            })?;
        let base_url = base_url
            .map(|s| s.to_string())
            .or_else(|| env::var("LLM_BASE_URL").ok())
            .ok_or_else(|| {
                HelloAgentException::config("必须提供服务地址 (base_url 或 LLM_BASE_URL)")
            })?;
