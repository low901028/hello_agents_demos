...
use super::exceptions::HelloAgentException;
use super::llm_resp_req::*;
...
impl OpenAIAdapter {
    ...
    /// 构造 ChatCompletionRequest 并序列化为 JSON
    fn build_request_body(
        ...
        for (k, v) in kwargs {
            match k.as_str() {
                ...
                _ => { request.extra.insert(k, v); }
            }
        }

        // 添加 tools 和 tool_choice
        if let Some(tools) = tools {
            // tools 参数已经是 Vec<HashMap<String, Value>>，需要转换为 Vec<ToolDefinition>
...
impl LLMAdapter for OpenAIAdapter {
    ...
    fn invoke(
        ...
        // 将 kwargs 转换为 Value 类型的 HashMap
    ...
    > {
        ...
        if !resp.status().is_success() {
            let status = resp.status();
            let text = resp.text().unwrap_or_default();
            return Err(HelloAgentException::llm(format!(
                "API 错误 ({}): {}",
                status, text
            )));
        }

        // 读取整个响应体，避免惰性阻塞导致的 tokio 冲突
        let text = resp.text()?;
    ...
    fn invoke_with_tools(
        ...
        let body = self.build_request_body(messages, kwargs, false, Some(tools), None);
...
pub fn create_adapter(
    ...
    timeout: i32,
    model: &str,
) -> Result<Box<dyn LLMAdapter>, HelloAgentException> {
