...
// 可观测性使用示例（完全异步，使用 tokio::sync::Mutex）
...
impl TraceLogger {
    ...
    /// 记录事件（异步锁，I/O 在锁外）
    pub async fn log_event(&self, event_type: &str, payload: Value, step: Option<usize>) {
        // 1. 构造事件（不在锁内）
        let mut event = json!({
        ...
        // 3. 写入 JSONL 文件（锁外，同步 I/O 不阻塞异步任务）
    ...
    /// 完成并生成统计报告（异步获取锁）
    pub async fn finalize(&self) -> Value {
...
async fn demo_basic_logging() -> Result<(), HelloAgentError> {
    println!("{}", "=".repeat(60));
    println!("示例 1: 基本事件记录");
    println!("{}", "=".repeat(60));
    ...
    logger.log_event("tool_call", json!({
        "tool_name": "Calculator",
        "parameters": {"expression": "2 + 3"}
    }), Some(1)).await;

    logger.log_event("tool_result", json!({
        "tool_name": "Calculator",
        "status": "success",
        "result": "5"
    }), Some(1)).await;

    logger.log_event("llm_response", json!({
        "content": "计算结果是 5",
        "tokens": 10
    }), Some(1)).await;

    logger.log_event("session_end", json!({
        "final_answer": "计算完成",
        "total_steps": 1
    }), None).await;

    let stats = logger.finalize().await;
    ...
    println!("✅ 事件数量: {}", stats["total_steps"]);
    Ok(())
}

async fn demo_sanitization() -> Result<(), HelloAgentError> {
    println!("\n{}", "=".repeat(60));
    println!("示例 2: 敏感信息脱敏");
    println!("{}", "=".repeat(60));
    ...
    logger.log_event("config_loaded", json!({
...
async fn demo_error_tracking() -> Result<(), HelloAgentError> {
    println!("\n{}", "=".repeat(60));
    println!("示例 3: 错误追踪");
    println!("{}", "=".repeat(60));
    ...
    logger.log_event("session_start", json!({"agent_name": "ErrorDemo"}), None).await;
    logger.log_event("tool_call", json!({"tool_name": "Read"}), Some(1)).await;
    logger.log_event("tool_result", json!({"tool_name": "Read", "status": "success"}), Some(1)).await;
    logger.log_event("tool_call", json!({"tool_name": "Write"}), Some(2)).await;
    logger.log_event("tool_result", json!({
        "tool_name": "Write",
        "status": "error",
    ...
    logger.log_event("circuit_breaker", json!({
        "tool_name": "Write",
        "action": "opened",
        "reason": "连续失败 3 次"
    }), Some(3)).await;
    logger.log_event("session_end", json!({"status": "error", "error": "工具执行失败"}), None).await;
...
async fn demo_statistics() -> Result<(), HelloAgentError> {
    println!("\n{}", "=".repeat(60));
    println!("示例 4: 统计信息");
    println!("{}", "=".repeat(60));
    ...
    logger.log_event("session_start", json!({"agent_name": "StatsDemo"}), None).await;
    for step in 1..=5 {
        logger.log_event("tool_call", json!({"tool_name": format!("Tool{}", step)}), Some(step)).await;
        logger.log_event("tool_result", json!({"tool_name": format!("Tool{}", step), "status": "success"}), Some(step)).await;
        logger.log_event("model_output", json!({
            "usage": {
    ...
    logger.log_event("session_end", json!({"total_steps": 5}), None).await;

    let stats = logger.finalize().await;

    println!("\n统计信息:");
    println!("  总步数: {}", stats["total_steps"]);
    println!("  工具调用次数: {}", stats["tool_calls"].as_object().map_or(0, |m| m.len()));
    println!("  总 Tokens: {}", stats["total_tokens"]);
    println!("  总成本: ${:.4}", stats["total_cost"].as_f64().unwrap_or(0.0));
    println!("  错误数: {}", stats["errors"].as_array().map_or(0, |a| a.len()));
    println!("  会话时长: {:.2}s", stats["duration_seconds"].as_f64().unwrap_or(0.0));
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    demo_sanitization().await?;
    demo_error_tracking().await?;
